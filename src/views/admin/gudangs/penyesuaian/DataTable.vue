            ...
            <template v-for="(item, index) in data" :key="item.id">
              ...
              <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td
                  ...
                  <div v-if="item?.tanggal_gudang">{{ formatDateIndo(item?.tanggal_gudang)
                    }}, {{ formatTimeIndo(item?.tanggal_gudang) }}</div>
                  <div v-else>-</div>
                </td>
                <td
                  class="border border-gray-800 dark:border-gray-500 p-3  text-right text-gray-700 dark:text-gray-300">
                  {{
                    item?.stock_akhir_gudang }}</td>
                <td
...
function updateSelisih(index) {
  ...
  item.selisih = item.stock_fisik - item.stock_akhir_gudang
