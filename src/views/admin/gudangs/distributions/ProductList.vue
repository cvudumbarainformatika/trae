<template>
  <div
    class="bg-white dark:bg-secondary-800 rounded-xl shadow-sm p-4 h-full transition-all duration-300 hover:shadow-md">
    <h3 class="text-lg font-medium text-secondary-900 dark:text-white mb-4 flex items-center justify-between">
      <span class="flex items-center">
        <Icon name="ShoppingBasket" class="mr-2 text-primary-500" /> Item Distribusi
      </span>
    ...
    <div class="relative mb-4">
      <div class="flex gap-2">
        <div class="flex-1">
          <SearchDropdown v-model="productSearchModel" ref="productSearchRef"
            ...
            api-url="/api/v1/products/search" api-response-path="data.data" :api-params="{ per_page: 10 }"
            :use-api="true" @select="addProductToOrder" @items-loaded="onProductsLoaded">
            <template #item="{ item }">
              <div class="font-medium text-secondary-900 dark:text-white">{{ item.name }}</div>
              <div class="flex justify-between text-sm">
                <span class="text-secondary-500 dark:text-secondary-400">{{ item.barcode || 'Tidak ada barcode'
                  }}</span>
                <span class="font-medium text-primary-600 dark:text-primary-400">Rp {{ formatNumber(item.hargabeli)
                  }}</span>
              </div>
            </template>
          </SearchDropdown>
    ...
    <BarcodeScanner v-if="showScanner" @scan-success="handleBarcodeScan" @scan-error="showScanner = false"
    ...
    <div class="space-y-3 max-h-[400px] overflow-y-auto pr-2">
      ...
      <div v-if="items.length === 0"
        class="text-center py-12 text-secondary-400 dark:text-secondary-500 bg-secondary-50 dark:bg-secondary-800/50 rounded-lg border-2 border-dashed border-secondary-200 dark:border-secondary-700">
        ...
        <p class="text-sm max-w-md mx-auto">
          Cari dan tambahkan produk ke Distribusi Produk Anda ke Toko menggunakan kotak pencarian di atas
        </p>
        <!-- <div class="mt-4">
      ...
      <div v-for="(item, index) in items" :key="index"
        class="p-3 bg-secondary-50 dark:bg-secondary-700 rounded-lg border-l-4 border-primary-500 animate-fadeIn relative">
        <!-- Header: Product name, barcode, and delete button -->
        <div class="flex justify-between items-start mb-2">
          <div class="flex-1">
            <div class="font-medium text-secondary-900 dark:text-white">{{ item.product?.name || 'Tidak ada nama produk'
              }}
            </div>
          ...
          <div class="flex flex-col items-end ">
            <button @click="removeItem(index)"
              class="text-red-500 hover:text-red-600 dark:hover:text-red-400 ml-2 absolute bottom-2">
        ...
        <div class="flex justify-between items-center mt-1">
          <div class="flex items-center gap-2">
            <!-- <div class="relative">
              <span
                class="absolute left-2 top-1/2 -translate-y-1/2 text-xs text-secondary-500 dark:text-secondary-400">Rp</span>
              <input v-model.number="item.price" type="number" min="0" @input="updateItemTotal(index)"
    ...
    <div class="mt-6 pt-4 border-t border-secondary-200 dark:border-secondary-700">
      <div class="flex justify-between items-center font-medium text-lg">
        ...
        <span class="text-primary-600 dark:text-primary-400">{{ formatNumber(calculateTotal()) }} Item</span>
      </div>
    </div>
  </div>
</template>
...
const incrementQuantity = (index) => {
  const currentQuantity = props.items[index].qty
  emit('update-quantity', { index, qty: currentQuantity + 1 })
}
