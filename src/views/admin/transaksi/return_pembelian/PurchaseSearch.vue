<template>
  <div class="">
    <SearchDropdown ref="searchRef" id="product-search" v-model="searchQuery"
      ...
      item-label="name" not-found-text="Data Pembelian tidak ditemukan" not-found-subtext="Coba kata kunci lain"
      :show-add-button="false" api-url="/api/v1/purchases/search" api-response-path="data.data"
      ...
      <template #item="{ item }">
        ...
        <div v-for="(item, index) in item.items" :key="index">
          <div class="text-primary-600 dark:text-primary-400">
            <div>{{ item.product?.name }} <span class="text-red-600">({{ formatRupiah(item?.price) }} x {{ item.qty
                }})</span> = Rp {{
