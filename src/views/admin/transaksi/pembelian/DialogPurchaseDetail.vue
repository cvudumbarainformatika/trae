<template>
  <div class="flex flex-col gap-6 p-4 bg-gradient-to-br from-secondary-50 to-secondary-100 dark:from-secondary-900 dark:to-secondary-800">
      ...
      <div>
        ...
        <div class="flex items-center">
          <div class="w-8 h-8 rounded-full flex items-center justify-center text-white"
            ...
            <Icon :name="(purchase.due_amount || purchase.total - (purchase.paid_amount || 0)) <= 0 ? 'CheckCircle' : 'AlertCircle'" class="w-5 h-5" />
          </div>
          <div class="ml-3 text-lg font-semibold text-secondary-900 dark:text-white capitalize">
            {{ (purchase.due_amount || purchase.total - (purchase.paid_amount || 0)) <= 0 ? 'Lunas' : 'Hutang' }}
          </div>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="p-4 bg-white dark:bg-secondary-800 rounded-xl shadow-sm md:col-span-2">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
          ...
          <div class="text-secondary-900 dark:text-white font-medium capitalize">{{ purchase.payment_method || '-' }}</div>
    ...
    <div class="p-4 bg-white dark:bg-secondary-800 rounded-xl shadow-sm">
        ...
        <table class="min-w-full text-sm">
          <thead class="bg-secondary-50 dark:bg-secondary-700/50">
            <tr>
              ...
              <th class="px-4 py-3 text-right font-semibold text-secondary-600 dark:text-secondary-300">Harga Satuan</th>
              <th class="px-4 py-3 text-right font-semibold text-secondary-600 dark:text-secondary-300">Subtotal</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-secondary-200 dark:divide-secondary-700">
            <tr v-for="item in purchase.items" :key="item.id || item.product_id" class="hover:bg-secondary-50 dark:hover:bg-secondary-700/30">
              <td class="px-4 py-3 text-secondary-700 dark:text-secondary-200">{{ item.product?.name || item.product_name || '-' }}</td>
              <td class="px-4 py-3 text-right text-secondary-700 dark:text-secondary-200">{{ item.qty }}</td>
              <td class="px-4 py-3 text-right text-secondary-700 dark:text-secondary-200">{{ formatCurrency(item.price) }}</td>
              <td class="px-4 py-3 text-right text-secondary-700 dark:text-secondary-200">{{ formatCurrency(item.subtotal) }}</td>
            </tr>
      ...
      <!-- Totals -->
      <div class="mt-4 pt-4 border-t border-secondary-200 dark:border-secondary-700 flex flex-col items-end gap-1 text-sm">
        <div class="flex justify-between w-full max-w-xs">
          <span class="text-secondary-600 dark:text-secondary-300">Subtotal:</span>
          <span class="font-medium text-secondary-800 dark:text-secondary-100">{{ formatCurrency(purchase.subtotal || purchase.total) }}</span>
        </div>
        <div v-if="purchase.discount_amount > 0" class="flex justify-between w-full max-w-xs">
          <span class="text-secondary-600 dark:text-secondary-300">Diskon ({{ purchase.discount_percentage || 0 }}%):</span>
          <span class="font-medium text-secondary-800 dark:text-secondary-100">- {{ formatCurrency(purchase.discount_amount) }}</span>
        </div>
        <div v-if="purchase.tax_amount > 0" class="flex justify-between w-full max-w-xs">
          ...
          <span class="font-medium text-secondary-800 dark:text-secondary-100">+ {{ formatCurrency(purchase.tax_amount) }}</span>
        </div>
        <div class="flex justify-between w-full max-w-xs mt-1 pt-1 border-t border-secondary-200 dark:border-secondary-700">
          <span class="font-bold text-secondary-800 dark:text-secondary-100">Total Akhir:</span>
          <span class="font-bold text-lg text-primary-600 dark:text-primary-400">{{ formatCurrency(purchase.total) }}</span>
        </div>
        <div class="flex justify-between w-full max-w-xs mt-2">
          ...
          <span class="font-medium text-green-600 dark:text-green-400">{{ formatCurrency(purchase.paid_amount || 0) }}</span>
        </div>
        <div class="flex justify-between w-full max-w-xs">
          <span class="text-secondary-600 dark:text-secondary-300">Sisa Hutang:</span>
          <span class="font-medium text-red-600 dark:text-red-400">{{ formatCurrency(purchase.due_amount || purchase.total - (purchase.paid_amount || 0)) }}</span>
        </div>
      </div>
    </div>
  </div>
