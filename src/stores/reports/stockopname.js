...
export const useStockOpnameStore = defineStore('stock-opname-store', {
  state: () => ({
    items: [],
    meta: null,
    params: {
  ...
  actions: {
    async fetchData() {
      this.loading = true
      try {
        const { data } = await api.get('/api/v1/reports/stock-opname', {
          params: this.params
        })
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useStockOpnameStore, import.meta.hot))
}
