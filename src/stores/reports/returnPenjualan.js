...
export const useReportReturnPenjualanStore = defineStore('report-return-penjualan-store', {
  state: () => ({
    items: [],
    summary:null,
    meta: null,
  ...
  getters: {
    ...
    header: (state) => {
      return {
        ...
        subtitle: 'Laporan Return Penjualan berdasarkan periode dan pendukung lainnya',
  ...
  actions: {
    async fetchData() {
      this.loading = true
      try {
        const { data } = await api.get('/api/v1/reports/return-penjualan', {
          params: this.params
        })
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useReportReturnPenjualanStore, import.meta.hot))
}
