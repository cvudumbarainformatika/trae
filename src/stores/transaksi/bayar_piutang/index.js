...
export const useBayarPiutangStore = defineStore('bayar-piutang-store', {
  state: () => ({
    items: [],
    meta: null,
    params: {
  ...
  actions: {
    async fetchData() {
      this.loading = true
      try {
        const { data } = await api.get('/api/v1/pembayaran-piutang', {
          params: this.params
        })
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useBayarPiutangStore, import.meta.hot))
}
