...
export const useStockGudangStore = defineStore('stock-gudang-store', {
  state: () => ({
    items: [],
    meta: null,
    header:{
  ...
  actions: {
      ...
      try {
        ...
        if (this.items.length > 0) {
          this.items = this.items.map((item) => ({
            ...item,
            stock_fisik: item.stok_fisik ?? item.stock_akhir_gudang ?? 0,
            selisih: (item.stok_fisik ?? item.stock_akhir_gudang ?? 0) - item.stock_akhir_gudang,
            isEdited: false
          }))
        }
    ...
    async fetchStockById(id, params) {
      this.loading = true
      try {
        const { data } = await api.get(`/api/v1/products/mutations/gudang/${id}`, {
          params
        })
    ...
    async savePenyesuaian(item, index) {
      console.log('item penyesuaian', item, index);

      this.loading = true
      try {
        const { data } = await api.post(`/api/v1/products/stock-opname/gudang`, item)
    ...
    canceled(item, index) {
      ...
      const oldStockSistem = item.stock_akhir_gudang
      const oldSelisih = oldStockFisik - oldStockSistem

      this.items[index].stock_fisik = oldStockFisik
      this.items[index].stock_akhir_gudang = oldStockSistem
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useStockGudangStore, import.meta.hot))
}
