    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex space-x-2">
        <button @click="changePage(1)" :disabled="pagination.currentPage <= 1"
          class="inline-flex items-center px-2 py-2 rounded-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600   font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
          :class="{ 'opacity-50 cursor-not-allowed': pagination.currentPage <= 1 }">
          <Icon name="chevrons-left" class="w-4 h-4" />
        </button>

        <button @click="changePage(pagination.currentPage - 1)" :disabled="pagination.currentPage <= 1"
          class="inline-flex items-center px-4 py-2 rounded-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600   font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
          :class="{ 'opacity-50 cursor-not-allowed': pagination.currentPage <= 1 }">
        ...
        <div class="hidden md:flex space-x-1">
          <template v-for="page in pagination.totalPages" :key="page">
            <button v-if="page >= pagination.startPage && page <= pagination.endPage" @click="changePage(page)"
              class="inline-flex items-center justify-center w-10 h-10 rounded-full   font-medium transition-colors"
        ...
        <button @click="changePage(pagination.totalPages)" :disabled="pagination.currentPage >= pagination.totalPages"
          class="inline-flex items-center px-2 py-2 rounded-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600   font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
          :class="{ 'opacity-50 cursor-not-allowed': pagination.currentPage >= pagination.totalPages }">
